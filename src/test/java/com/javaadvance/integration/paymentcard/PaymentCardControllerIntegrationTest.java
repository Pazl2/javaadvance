package com.javaadvance.integration.paymentcard;
...
class PaymentCardControllerIntegrationTest extends BaseIntegrationTest {
    ...
    @Test
    void shouldReturnFilteredCardsByHolder() {

        UserResponse user = createUser("filtercards@example.com");
        assertThat(user).isNotNull();

        PaymentCardCreateRequest card1 = buildCard("4111111111111111");
        card1.setHolder("IVAN IVANOV");

        PaymentCardCreateRequest card2 = buildCard("5500005555555559");
        card2.setHolder("MARIA SMITH");

        restTemplate.postForEntity("/users/" + user.getId() + "/cards", card1, PaymentCardResponse.class);
        restTemplate.postForEntity("/users/" + user.getId() + "/cards", card2, PaymentCardResponse.class);

        ResponseEntity<String> response =
                restTemplate.getForEntity("/cards?holder=IVAN&page=0&size=10", String.class);
